...
fn main() -> anyhow::Result<()> {
    ...
    println!(
        "{}",
        serde_json::json!({"scope":"warm 256x256, 2-step full generation", "backend":format!("{backend:?}"), "selection":pipeline.fusion_selection(), "median_ms":samples[3], "samples_ms":samples})
    );
    drop(pipeline);
    anyhow::ensure!(
