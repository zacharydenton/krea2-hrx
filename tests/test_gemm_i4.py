"""The three INT4 GEMM epilogues against a float64 oracle, and against a reference kernel.
...
(GEMM_I4_BASELINE, a Git revision, default HEAD) must be bit-exact, which is how a
...
def oracle(mode, a, w, w_scale, a_scale, gate, residual):
    ...
    if mode == "plain":
        return full
    if mode == "resid":
        return residual.astype(np.float64) + gate[None, :] * full
...
def run(mode, source, symbol, namespace, m, k, n, kpad, operands, td, tile):
    ...
    if mode == "resid":
        args += [("inout_f16", (residual, (m, n))), ("in", gate)]
    else:
...
def main():
    baseline_rev = os.environ.get("GEMM_I4_BASELINE", "HEAD")
    ...
    with workdir() as td:
        ...
        for mode in modes:
            ...
            baseline = None
            try:
                text = subprocess.check_output(["git", "show", f"{baseline_rev}:{source}"], cwd=ROOT, stderr=subprocess.DEVNULL)
