"""Smoothed INT4 QK kernel and its INT8 twin: correctness and timings include native preprocessing."""
import os
import json
import math
...
def quantize(x, levels=7):
    scale = x.abs().amax(-1, keepdim=True).clamp_min(1e-30) / levels
...
def oracle(q, k, v, levels=7):
    ...
    for start in range(0, q.shape[1], 128):
        ...
        scores = quantize(a - mean, levels) @ quantize(kc, levels).transpose(1, 2) + mean @ kc.transpose(1, 2)
...
def main():
    with tempfile.TemporaryDirectory() as tmp:
        for tokens in (16, 48, 65, 100, 4115):
            ...
            for bits, levels in ((4, 7), (8, 127)):
                want = oracle(q, k, v, levels)
                out, timing = run(q, k, v, Path(tmp), f"attention_sage_i{bits}_fast")
                cs = cosine(out, want)
                print(json.dumps(dict(tokens=tokens, bits=bits, cosine_vs_oracle=cs, **timing)), flush=True)
                assert torch.isfinite(out).all() and cs > 0.9999
                pp, pt = run(q, k, v, Path(tmp), f"attention_sage_i{bits}_fast_prefetch")
                print(json.dumps(dict(tokens=tokens, bits=bits, prefetch_exact=bool(torch.equal(pp, out)), **pt)), flush=True)
