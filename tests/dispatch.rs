...
fn prepared_operations_reuse_both_shapes_and_remain_ordered() {
    ...
    let prepared = krea2::kernels::cache::PreparedKernels::default();
    for count in [257usize, 1009, 257, 1009] {
        let grid = (count.div_ceil(256) as u32, 1);
        let kernel =
            prepared.get(&stream, "unary_one", krea2::kernels::Config::new(), grid).unwrap();
...
fn a_cached_kernel_outlives_the_cache_that_made_it() {
    ...
    let kernel = {
        let prepared = krea2::kernels::cache::PreparedKernels::default();
        let config = krea2::kernels::config([("count_b", COUNT as u64)]);
...
fn compiler_cache_separates_device_targets() {
    ...
    let a = krea2::kernels::cache::compiler_for_target(None, &first).unwrap();
    let b = krea2::kernels::cache::compiler_for_target(None, &second).unwrap();
    assert_eq!(a.target(), &first);
    assert_eq!(b.target(), &second);
    assert_eq!(
        krea2::kernels::cache::compiler_for_target(None, &first).unwrap().target(),
        &first
    );
}
