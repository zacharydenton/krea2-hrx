...
# ///
"""The Krea 2 transformer in plain torch, optionally with the W8A8 ConvRot arithmetic.

`scripts/parity.sh` answers "how far is this host from the unquantized model". It cannot
answer the follow-up -- how much of that is the quantization scheme and how much is our
implementation of it -- because it has only two points to compare. This supplies the third:
the same int8 ConvRot checkpoint run by a transcription of diffusers' Krea2Transformer2DModel
onto the ComfyUI key names, with no kernel of ours involved.

    ./scripts/quantized_reference.py forward    --checkpoint CKPT --quant w8a8 --out V.npy
    ./scripts/quantized_reference.py trajectory --checkpoint CKPT --quant none --out L.npy
...
Measured this way at 1024x1024 on the pinned fixture, one forward, everything on the GPU:
    ...
    quantization scheme, torch W8A8 against the identical torch code unquantized   0.0129
    our kernels against that scheme                                                0.0117
    two independent unquantized implementations (this transcription vs diffusers)  0.0070

So the scheme costs about as much as our deviation from it, and our deviation is under twice
the floor between any two honest implementations of the same maths. Decoded through one
untiled VAE the scheme reaches 30.93 dB against the reference and our kernels 31.13 dB.

Weights are the ComfyUI-format files, not the diffusers repository: `quant="w8a8"` needs the
int8 checkpoint's packed rows and `weight_scale` tensors, which the diffusers export does not
carry. Slow by construction -- dequantisation and the Hadamard rotations run in eager torch,
about two minutes per forward at 1024x1024 -- so it is a diagnostic, not a gate.
"""
...
def main() -> int:
    ...
    ap.add_argument("mode", choices=["forward", "trajectory"])
    ap.add_argument("--checkpoint", required=True,
                    help="a ComfyUI-format checkpoint; int8 ConvRot for --quant w8a8")
    ap.add_argument("--quant", default="w8a8", choices=["none", "w8a8", "w4a4"])
    ap.add_argument("--fixture", type=Path, default=root / "build/quality",
                    help="supplies the noise, conditioning and schedule")
    ap.add_argument("--out", type=Path, required=True)
    ap.add_argument("--device", default="cuda")
    a = ap.parse_args()

    meta = json.loads((a.fixture / "job.json").read_text())
    ...
    def sigma(k):
        if k == steps:
            return np.float32(0)
        raw = np.float32(1.0 - k / steps)
        return shift / (shift + (np.float32(1) / raw - np.float32(1)))

    # The int8 path indexes packed rows directly, so the weights must already be resident.
    weights = load_file(str(a.checkpoint), device=a.device)
    ref = Krea2Ref(weights, quant=a.quant, device=a.device, dtype=torch.bfloat16)
    text = torch.from_numpy(np.load(a.fixture / "text.npy")).to(a.device).bfloat16()[None]
    state = torch.from_numpy(np.load(a.fixture / "noise.npy")).to(a.device).bfloat16()[None]
    grid = px // 16

    def evaluate(x, s):
        t = torch.tensor([float(s)], device=a.device, dtype=torch.bfloat16)
        return ref.forward(x, text, t, grid, grid)

    with torch.no_grad():
        if a.mode == "forward":
            out = evaluate(state, sigma(0))
        else:
            for k in range(steps):
                s = sigma(k)
                velocity = evaluate(state, s)
                # kernels/native/euler.loom rounds the delta, the product and the sum.
                delta = torch.tensor(float(sigma(k + 1) - s)).bfloat16().float()
                product = (delta * velocity.bfloat16().float()).bfloat16().float()
                state = (state.float() + product).bfloat16()
                print(f"  step {k + 1}/{steps}", flush=True)
            out = state
    np.save(a.out, out.float().cpu().numpy())
    print(f"{a.mode} ({a.quant}) -> {a.out}  {tuple(out.shape)}")
    return 0
