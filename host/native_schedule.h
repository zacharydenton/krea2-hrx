...
inline float dynamic_mu(int image_tokens) {
  ...
  return float(image_tokens * m + b);
}
inline float scheduler_sigma(int step, int steps, float mu = 1.15f) {
  ...
  const float shift = float(std::exp(double(mu)));
