"""kernels/gemm_i4{,_resid,_swiglu}_256.loom: Krea 2's W4A4 GEMM family on the 256x128
workgroup tile (eight 64x64 wave tiles, 0.5 LDS operand reads per multiply), with the
three epilogues the blocks need, arithmetic identical to the 128x128 kernels:
...
A [M][K] and W [N][K] are int4 nibbles (low first), i32 accumulation, f32 scales.

    python3 tools/gen_gemm.py            writes the three kernels (then loom-format them)
...
PACKETS = (TM + TN) * 4 // 256
A_PACKETS = TM * 4 // 256
KSTEP, KQ, SUBS = 128, 8, 8   # k per step, k per i32 quad, 16-wide sub-steps per step


def generate(mode: str, loads: str = "plain", raster: str = "shorten") -> str:
    ...
    assert mode in ("plain", "resid", "swiglu") and loads in ("first", "plain", "predicated") and raster in ("shorten", "pad")
...
"""
    ...
    K = head + f"""// A [M][K] and W [N][K] are int4 nibbles (low first), both Hadamard-rotated along K.
...
"""
    ...
    K += f"""  %i4_schema = encoding.define #encoding.operand<element_format=i4, payload_elements=16, payload_registers=2> : encoding<schema>
