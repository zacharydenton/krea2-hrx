...
def pack_tail(bits: int) -> str:
    ...
    (low first), int8 as two i32 of bytes (low first)."""
    ...
    if bits == 4:
        ...
        out += "    view.store %q_o7, %qw_view[%row, %q_w] : i32, view<[%tokens_b]x[%out_words]xi32>\n"
    else:
...
def kernel(name: str, bits: int = 4) -> str:
    """bits 4: codes -7..7, eight nibbles per i32 (the W4A4 GEMMs); bits 8: codes -127..127, four bytes
    per i32 (the W8A8 GEMMs). The row arithmetic, rotation and scale are otherwise the same."""
    f = FORM[name]
    lds = "f16" if name == "plain" else "f32"
    lds_bytes = 2 if lds == "f16" else 4
    ns, sym = f"krea2.prepare_{name}_i{bits}", f"krea2_prepare_{name}_i{bits}"
    levels, mask, per_word = (7, 15, 8) if bits == 4 else (127, 255, 4)
...
// token's absmax, and write symmetric int{bits} (q = round(x / s), s = absmax / {levels},
// {"nibbles" if bits == 4 else "bytes"} low first) with the f32 scale beside it -- the operand the int{bits} GEMM consumes.
//
...
// packed output row pitch in elements (the GEMM's k_stride): width, or width plus one k step when the row is 8192 bytes
config.decl @{ns}.out_stride : %value: index where [range(%value, 2048, 65536), mul(%value, {128 if bits == 4 else 64})]
{eps_cfg}{extra_cfg}
...
{eps_get}{extra_get}  %c0 = index.constant 0 : index
  ...
  %seven = scalar.constant {levels}.0 : f32
  %neg_seven = scalar.constant -{levels}.0 : f32
  ...
  %fifteen = scalar.constant {mask} : i32
  ...
  %word_width = index.div %width, %c{per_word} : index
  %word_last = index.sub %word_width, %c2 : index
  %out_words0 = index.div %out_stride, %c{per_word} : index
...
def uniform_loops(text: str) -> str:
    ...
    return text.replace("  %quads = index.div %width, %c4 : index\n",
                        ...
                        "  %quads_per_lane = index.div %quads, %c256 : index\n  %chunks_per_lane = index.div %chunk_count, %c256 : index\n")

for bits in (4, 8):
    for name in FORM:
        (OUT / f"prepare_{name}_i{bits}.loom").write_text(uniform_loops(lds_type(kernel(name, bits), "f16" if name == "plain" else "f32")))
