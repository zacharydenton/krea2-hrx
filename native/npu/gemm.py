...
def _build_design(
    ...
    *,
    ...
    if (dev_str, dtype_in_str, dtype_out_str, b_col_maj, c_col_maj, scalar,
        emulate_bf16_mmul_with_bfp16) != ("npu2", "bf16", "f32", 1, 0, False, False):
        raise ValueError("This project implements only native BF16 NT -> F32 on XDNA2")
    if os.environ.get("HRX_AIE_BACKEND") != "chess" or not use_chess:
        raise ValueError("This project is compiled only with Chess")
    matmul_kernel = ExternalFunction(
        "matmul_bf16_f32", source_file=str(Path(__file__).with_name("mm.cc")),
        arg_types=[np.ndarray[(m*k,), np.dtype[dtype_in]],
                   np.ndarray[(k*n,), np.dtype[dtype_in]],
                   np.ndarray[(m*n,), np.dtype[dtype_out]]],
        compile_flags=[f"-DDIM_M={m}", f"-DDIM_K={k}", f"-DDIM_N={n}",
                       "-Dbf16_f32_ONLY", "-DB_COL_MAJ"],
        include_dirs=[str(Path(os.environ["AIETOOLS_ROOT"]) / "include")],
        use_chess=True,
    )
    zero_kernel = Kernel("zero_f32", matmul_kernel.object_file_name,
                         [np.ndarray[(m*n,), np.dtype[dtype_out]]])
    r, s, t = 4, 8, 8
    assert m % r == 0
    assert k % s == 0
    ...
    # The outer output stride is m * n_aie_rows * N elements. XDNA2's
    # shim descriptor allows at most 2^20; use one row block when it would
    # overflow so the singleton dimension has stride zero instead.
    tb_max_n_rows = 2 if c_col_maj or m * n_aie_rows * N > 1 << 20 else 4
