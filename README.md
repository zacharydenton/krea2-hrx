...
- **`build/krea2`**: the CLI, a small Rust program over the C ABI (`cli/`), which is
  ...
  has the rest.
- **`libkrea2_pipeline.so`** with `build/include/krea2_pipeline.h`: the same pipeline
  behind a C ABI for any language with FFI (tokenizer, text encoder, transformer, VAE,
  `krea2_generate` and `krea2_generate_guided`).
- **`libkrea2.so`** with `build/include/krea2.h` and [krea2_loom.py](krea2_loom.py):
  the 28 blocks alone, resident per sequence length, for use inside diffusers
  (`tools/pipeline.py --backend loom`). It is the same file as the pipeline library
  under a second name, so a process that loads both APIs gets one HRX runtime. The
  headers are build artifacts, generated from the Rust sources by cbindgen; none is
  committed.
...
| `cli/` | the CLI over the C ABI, and with it the example of using that ABI |
